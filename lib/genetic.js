var Genetic = Genetic || (function(){
	...
	function Genetic() {
        ...
        let pop = [];
				if (!this.usingWebWorker && this.configuration.workerPath && this.configuration.workersCount > 0) {
            ...
            entitiesChunks.push(this.entities.slice(i, i + chunkSize));
					...
					const promises = entitiesChunks.map(async function(entities) {
						return new Promise(function(resolve) {
							const myWorker = self.startWorker(self.configuration.workerPath, { entities, userData: self.userData }, (err, result) => {
