...
var Genetic = Genetic || (function(){
	...
	function Genetic() {
		...
		this.mapFitnessAsync = function(entities, callback) {
			...
			var scores = entities.map(function() {
				return null;
			});
			var nbScores = 0;
			...
			entities.forEach(function(entity, i) {
				...
				setTimeout(function() {
					...
					if (self.fitness.length === 1) {
						setScore(self.fitness(entity));
					} else {
						self.fitness(entity, setScore);
					}
				}, 0);
			});
		};
		...
		this.seriesAsync = function(nb, func) {
			...
			var recursion = function(i) {
				setTimeout(function() {
					if(i < nb) {
						func.call(self, function recursionCallback() {
							recursion(i + 1);
						});
					}
				}, 0);
		...
		this.start = function() {
			...
			this.seriesAsync(this.configuration.iterations, function runGeneration(generationCallback){
